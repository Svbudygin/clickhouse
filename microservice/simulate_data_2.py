...
TOTAL_EVENTS_GAUGE = Gauge('total_events_clickhouse', 'Total count of events in ClickHouse')
...
def simulate_event():
    ...
    media_files = (
        [f"https://cdn.example.com/{random.randint(100000, 999999)}.jpg" for _ in range(num_media_files)]
        if has_media
        else None
    )
    ...
    event = {
        ...
        "event": event_type,
        "state": "imported" if event_type == "SAVE" else ("updated" if event_type == "UPDATE" else "error"),
...
def compute_metrics():
    ...
    while True:
        try:
            ...
            update_count = client.execute("SELECT count() FROM product_load_events WHERE event='UPDATE'")[0][0]
            error_count = client.execute("SELECT count() FROM product_load_events WHERE event='ERROR'")[0][0]
            total_count = client.execute("SELECT count() FROM product_load_events")[0][0]
...
def register_to_consul():
    ...
    svc = {
        ...
        "Tags": ["metrics"],
        "Check": {
            "HTTP": f"http://{instance_id}:84/metrics",
            "Interval": "10s"
        }
    }
...
def main():
    ...
    while True:
        ...
        client.execute('INSERT INTO product_load_events (ts, item_id, event, state, attempt_id, data) VALUES', [
            (event_dt, event['item_id'], event['event'], event['state'], event['attempt_id'], event['data'])
        ])
